...
pattern = re.compile(
    r"([0-9]{4}-[0-9]{2}-[0-9]{2} [0-9]{2}:[0-9]{2}:[0-9]{2}(\.[0-9]{1,3})?) \| ((TRACE|DEBUG|INFO|NOTICE|WARN|WARNING|ERROR|SEVERE|FATAL))   \| nhhc\.settings:<module>:31 - ([A-Za-z0-9]+)",
    re.IGNORECASE,
)
            ...
            for log in logs:
                if match := re.match(pattern, log):
                    ...
                    with open(self.formatted_logfile, "w+") as output_log:
                        log_dict = {"Date_Time": date_time, "Log Type": log_type, "Message": message}
