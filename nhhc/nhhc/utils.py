...
def get_content_for_unauthorized_or_forbidden(request: HttpRequest) -> bytes:
    ...
    """
    return (
        bytes("You Must Be Logged In To Complete This Request", "utf-8")
        if not request.user.is_authenticated
        else bytes("You Must Be An Admin In To Complete This Request", "utf-8")
    )


def send_new_user_credentials(
    new_user_email: str, new_user_first_name: str, password, username
) -> HttpResponse:
    """
