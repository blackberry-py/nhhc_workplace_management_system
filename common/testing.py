...
def generate_mock_ZipCodeField() -> str:
    ...
    postal_code = str(secrets.randbelow(90000) + 10000)
...
def generate_mock_USSocialSecurityNumberField() -> str:
    ...
    while True:
        ...
        first_three_digits = secrets.randbelow(799) + 1
        ...
        middle_two_digits = secrets.randbelow(100)

        # Generates the last four digits of the SSN
        last_four_digits = secrets.randbelow(9999) + 1
...
def generate_random_encrypted_char():
    ...
    length = secrets.randbelow(60) + 1
    raw_char = "".join(secrets.choice(chars) for _ in range(length))
    # settings.ENCRYPT_PRIVATE_KEY
    public_key = settings.ENCRYPT_PUBLIC_KEY
    import_result = gpg.import_keys(public_key)
    if import_result.count == 0:
